...
public class Users_AddFavoriteDiagnosesGroup {
	public static Boolean users_AddFavoriteDiagnosesGroup_Test(HashMap<String, String> parameters, String strResponse,HashMap<String, Object> mapPostDetails, SoftAssert softAssert, String strQuery)
	{
			...
			if(Math.abs(countAfterPOST - ((int)mapPostDetails.get("countBeforePOST")))  == 1)
			{
				try
				{
					String strSQLQuery = "select top 1 " 
							+ " diag_favorite_name as GroupName "
							
							+  " from diagnosis_favorite_mstr "
							+  " WHERE user_id = '"+parameters.get("userId")+"' "
				...
				//	jsonList.add(Objects.equals(parameters.get("IsAutoReleasable"), "false") ? "N" : "Y");
					...
					jsonList.add(parameters.get("GroupName").toString());
