...
/*
 * ********************************************************************
 * Class Name						: ExternalSystems_GET
 ...
 * Purpose							: Route will fetch the details of the external systems configured
 * ******************************************************************
 */

public class ExternalSystems_GET
{
	public static Boolean ExternalSystems_GET_Test(String strResponse, String strUniqueJsonToken, SoftAssert softAssert)
	{
		return true;
	}
}

/*public class ExternalSystems_GET extends BaseTest_obsl
{
	SoftAssert softAssert = new SoftAssert();

	@SuppressWarnings("null")
	@Test
	public void externalSystems_GET()
	{
					...
					if (!strResponse.isEmpty())
					{
						if (!(strResponse.contains("The request is invalid")))
						{
							String strQuery = " select"
									+" external_system_id as id, external_system_name as externalSystemName"
									+" from external_system order by external_system_name";
							try
							{
								...
								if (obj instanceof JSONArray)
								{
									for(int j = 0; j< ((JSONArray) obj).size(); j++)
									{
										...
										jsonList.add(JsonPath.read(strResponse,"$..externalSystemName["+j+"]"));
									}
								}
