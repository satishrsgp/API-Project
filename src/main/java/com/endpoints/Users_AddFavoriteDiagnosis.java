...
public class Users_AddFavoriteDiagnosis {
	public static Boolean users_AddFavoriteDiagnosis_Test(HashMap<String, String> parameters, String strResponse,HashMap<String, Object> mapPostDetails, SoftAssert softAssert, String strQuery)
	{
		...
		try
		{
			...
			if(Math.abs(countAfterPOST - ((int)mapPostDetails.get("countBeforePOST")))  == 1)
			{
				try
				{
					String strSQLQuery =  "select top 1 diagnosis_code_id,diag_description from  diagnosis_favorite_xref"
					
					
							+ " where diag_favorite_id='"+parameters.get("groupId")+"' and created_by='"+parameters.get("userId")+"' order by create_timestamp desc ";
					...
					//jsonList.add(Objects.equals(parameters.get("StartDate"), null) ? null : parameters.get("StartDate").toString().replace("T", " ").trim());
				//jsonList.add(Objects.equals(parameters.get("EndDate"), null) ? null : (parameters.get("EndDate").toString()).replace("T", " "));
				//.add(parameters.get("Interval"));
				...
				//	jsonList.add(Objects.equals(parameters.get("IsAutoReleasable"), "false") ? "N" : "Y");
					...
					jsonList.add(parameters.get("IcdCode").toString());
					jsonList.add(parameters.get("Description").toString());
