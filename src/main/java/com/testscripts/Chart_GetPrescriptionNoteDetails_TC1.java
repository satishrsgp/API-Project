...
import com.endpoints.Chart_GetPrescriptionNoteDetails;
...
public class Chart_GetPrescriptionNoteDetails_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPrescriptionNoteDetails_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPrescriptionNoteDetails.chart_GetPrescriptionNoteDetails_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
