...
import com.endpoints.Master_GetTaskPriorities;
...
public class Master_GetTaskPriorities_TC1 extends BaseTest
{
		...
		try
		{
			...
			{	
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetTaskPriorities.master_GetTaskPriorities_Test(parameters, strResponse, "id", softAssert);
				}
				else
