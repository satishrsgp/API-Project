...
import com.endpoints.Master_GetPharmacyDetail;
...
public class Master_GetPharmacyDetail_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void master_GetPharmacyDetail_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Master_GetPharmacyDetail.master_GetPharmacyDetail_Test(parameters, strResponse, "pharmacyId", softAssert);
				}
				else
