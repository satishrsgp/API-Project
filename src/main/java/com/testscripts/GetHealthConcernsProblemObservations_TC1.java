...
import com.endpoints.GetHealthConcernsProblemObservations;
...
public class GetHealthConcernsProblemObservations_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void getHealthConcernsProblemObservations_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = GetHealthConcernsProblemObservations.getHealthConcernsProblemObservations_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
