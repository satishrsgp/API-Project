...
import com.endpoints.Chart_UpdatePatientProblem;
...
public class Chart_UpdatePatientProblem_TC1 extends BaseTest
{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdatePatientProblem.chart_UpdatePatientProblem_Test(parameters, softAssert);
					}
					else
					{
