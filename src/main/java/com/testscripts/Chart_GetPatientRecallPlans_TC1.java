...
import com.endpoints.Chart_GetPatientRecallPlans;
...
public class Chart_GetPatientRecallPlans_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientRecallPlans_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientRecallPlans.chart_GetPatientRecallPlans_Test(parameters, strResponse, "recallPlanId", softAssert);
				}
				else
				{
