...
import com.endpoints.Chart_DeleteObservationResult;
...
public class Chart_DeleteObservationResult_TC1 extends BaseTest
{
	...
	@Test
	public void chart_DeleteObservationResult_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DeleteObservationResult.chart_DeleteObservationResult(parameters, softAssert);
					}
					else
					{
