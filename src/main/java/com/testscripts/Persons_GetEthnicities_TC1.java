...
import com.endpoints.Persons_GetEthnicities;
...
public class Persons_GetEthnicities_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void persons_GetEthnicities_TC1()
	{
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestcaseStatus = Persons_GetEthnicities.persons_GetEthnicities_Test(parameters, strResponse, "personId", softAssert);

						//we are evaluating the person property details here
					/*	if(boolTestcaseStatus)
						{
							String strResponseDocumentPart = JsonPath.read(strResponse, "$..personId").toString();
							if(strResponseDocumentPart.startsWith("[") && strResponseDocumentPart.endsWith("]"))
								strResponseDocumentPart = strResponseDocumentPart.substring(1, strResponseDocumentPart.length()-1);
							boolTestDataStatus = Person_Documents_GET.Person_Documents_GET_Test(parameters, strResponseDocumentPart, "id", softAssert);
						}*/
					}
				}
