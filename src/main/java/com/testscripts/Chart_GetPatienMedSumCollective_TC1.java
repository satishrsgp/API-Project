...
import com.endpoints.Chart_GetPatienMedSumCollective;
...
public class Chart_GetPatienMedSumCollective_TC1 extends BaseTest
{
	...
	@Test
	public void chart_GetPatienMedSumCollective_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount(BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				mapRequestParrameters.put("strQueryString", "?collective=true&medicationId="+parameters.get("Med1")+"&medicationId="+parameters.get("Med2"));
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatienMedSumCollective.chart_GetPatienMedSumCollective_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
