...
import com.endpoints.Users_GetUserPhrases;
...
public class Users_GetUserPhrases_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void users_GetUserPhrases_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Users_GetUserPhrases.users_GetUserPhrases_Test(parameters, strResponse, "type", softAssert);
				}
				else
				{
