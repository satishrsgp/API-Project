...
import com.endpoints.Chart_GetInterventions;
...
public class Chart_GetInterventions_TC1 extends BaseTest {

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetInterventions_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetInterventions.Chart_GetInterventions_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
