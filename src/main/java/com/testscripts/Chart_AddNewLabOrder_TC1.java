...
import com.endpoints.Chart_AddNewLabOrder;
...
public class Chart_AddNewLabOrder_TC1 extends BaseTest 
{
	...
	@Test
	public void chart_AddNewLabOrder_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select order_num from lab_nor where person_id='"+parameters.get("personId")+"'";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_AddNewLabOrder.Chart_AddNewLabOrder_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
