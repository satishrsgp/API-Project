...
import com.endpoints.Chart_AddTrackingComment;
...
public class Chart_AddTrackingComment_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void chart_AddTrackingComment_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				//calculating the count before performing a POST
				//int countBeforePOST = DatabaseConnection.returnNumberOfRows(DatabaseConnection.statementExecution(BaseTest.connNGA, strCountQuery));
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 200)
				{
					boolTestDataStatus = Chart_AddTrackingComment.Chart_AddTrackingComment_Test(parameters, mapPostDetails, softAssert, strCountQuery);
				}
