...
import com.endpoints.Master_SearchDiagnoses;
...
public class Master_SearchDiagnoses_TC1 extends BaseTest {

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_SearchDiagnoses_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				mapRequestParrameters.put("strQueryString", "?searchText="+parameters.get("searchText"));
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_SearchDiagnoses.master_SearchDiagnoses_Test(parameters, strResponse, "rank", softAssert);
				}
				else
				{
