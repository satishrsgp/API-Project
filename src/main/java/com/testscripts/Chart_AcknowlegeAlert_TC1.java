...
public class Chart_AcknowlegeAlert_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void chart_AcknowlegeAlert_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select count(*) as count from patient_alerts_ehr_audit  where alert_id = '" + parameters.get("alertId") + " ' and created_by = '"+parameters.get(System.getProperty("LoggedInUserId")+"'");
				HashMap<String, Object> mapPostDetails = ComputePayLoadDetails.computePayLoadMap(parameters, strCountQuery, strSimpleClassName);
				mapPostDetails.clear();
				mapRequestParrameters.put("strPayLoad", "");
				
				
				String before_rowcount = DatabaseConnection.fetchColumnAsObject(connNGA, strCountQuery, "count").toString();
				
				
				//Sending the Request to get the Response
				objHandler = API_Request_RetObj.sendRequest(httpClient, mapRequestParrameters);
				
				String after_rowcount = DatabaseConnection.fetchColumnAsObject(connNGA, strCountQuery, "count").toString();
				...
				{	
					...
					{
						if(after_rowcount==before_rowcount+1){
