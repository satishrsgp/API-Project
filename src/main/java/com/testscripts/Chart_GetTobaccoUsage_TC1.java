...
import com.endpoints.Chart_GetTobaccoUsage;
...
public class Chart_GetTobaccoUsage_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetTobaccoUsage_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetTobaccoUsage.chart_GetTelephoneCalls_Test(parameters, strResponse, "encounterId", softAssert);
				}
				else
				{
