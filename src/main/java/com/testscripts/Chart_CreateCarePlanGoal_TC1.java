...
import com.endpoints.Chart_CreateCarePlanGoal;
...
public class Chart_CreateCarePlanGoal_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_CreateCarePlanGoal_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_CreateCarePlanGoal.Chart_CreateCarePlanGoal_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
