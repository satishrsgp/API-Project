...
import com.endpoints.Master_Codes_GET;
...
public class Master_Codes_GET_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_Codes_GET_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_Codes_GET.master_Codes_GET_Test(parameters, strResponse, "", softAssert);
				}
				else
				{
