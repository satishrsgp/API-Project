...
import com.endpoints.Persons_GetFormularies;
...
public class Persons_GetFormularies_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void persons_GetFormularies_TC1()
	{
		...
		Map<String, Object> mapRequestParrameters = RequestParameters.generateRequestParamsHashMap("GET", "", "?$orderby=medicationId");
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						String strResponse = objHandler.getStrResponse().toString().replace(".0", "");
						boolTestcaseStatus = Persons_GetFormularies.persons_GetFormularies_Test(parameters, strResponse, "medicationId", softAssert);
