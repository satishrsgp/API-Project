...
import com.endpoints.Chart_GetWastedVaccines;
...
/*
 * ********************************************************************
 * Class Name						: Chart_GetWastedVaccines
...
public class Chart_GetWastedVaccines_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetWastedVaccines_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetWastedVaccines.chart_GetWastedVaccines_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
