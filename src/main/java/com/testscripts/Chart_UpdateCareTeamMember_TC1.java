...
import com.endpoints.Chart_UpdateCareTeamMember;
...
public class Chart_UpdateCareTeamMember_TC1 extends BaseTest
{
	...
	@Test
	public void chart_UpdateCareTeamMember_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdateCareTeamMember.chart_UpdateCareTeamMember_Test(parameters, softAssert);
					}
					else
					{
