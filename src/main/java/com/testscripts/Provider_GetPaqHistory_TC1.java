...
import com.endpoints.Provider_GetPaqHistory;
...
public class Provider_GetPaqHistory_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void provider_GetPaqHistory_TC1()
	{
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestcaseStatus = Provider_GetPaqHistory.provider_GetPaqHistory(parameters, strResponse, "itemId", softAssert);
					}
				}
