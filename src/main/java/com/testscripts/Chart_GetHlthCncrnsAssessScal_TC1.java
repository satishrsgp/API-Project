...
import com.endpoints.Chart_GetHlthCncrnsAssessScal;
...
public class Chart_GetHlthCncrnsAssessScal_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetHlthCncrnsAssessScal_TC1()
	{
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestcaseStatus = Chart_GetHlthCncrnsAssessScal.chart_GetHlthCncrnsAssessScal(parameters, strResponse, "id", softAssert);
