...
import com.endpoints.Master_GetDiagnosisCategories;
...
public class Master_GetDiagnosisCategories_TC1 extends BaseTest{


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_GetDiagnosisCategories_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetDiagnosisCategories.master_GetDiagnosisCategories_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
