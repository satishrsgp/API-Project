...
import com.endpoints.Chart_GetPatientCareplanOutcomes;
...
public class Chart_GetPatientCareplanOutcomes_TC1 extends BaseTest {

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientCareplanOutcomes_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientCareplanOutcomes.chart_GetPatientCareplanOutcomes_Test(parameters, strResponse, "personId", softAssert);
				}
				else
				{
