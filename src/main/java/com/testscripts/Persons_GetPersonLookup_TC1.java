...
import com.endpoints.Persons_GetPersonLookup;
...
public class Persons_GetPersonLookup_TC1 extends BaseTest 
{
	...
	@Test
	public void persons_GetPersonLookup_TC1()
	{
		...
		try
		{
			...
			for(int k=2; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{	
				...
				mapQuery = GeneralUtils.generateQueryString(strSimpleClassName, k);
				mapRequestParrameters.put("strQueryString", mapQuery.get("QueryString"));
				...
				Object[] testexecutionResult = hashMapResult.get(String.valueOf(k-1));
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Persons_GetPersonLookup.persons_GetPersonLookup_Test(parameters, strResponse, "id", softAssert, mapQuery);
				}
				else
                ...
                hashMapResult = TestsEvaluation.updateHashMap(hashMapResult, k-1, boolTestDataStatus);
