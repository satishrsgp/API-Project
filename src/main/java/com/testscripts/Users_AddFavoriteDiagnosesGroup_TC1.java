...
import com.endpoints.Users_AddFavoriteDiagnosesGroup;
...
public class Users_AddFavoriteDiagnosesGroup_TC1 extends BaseTest
{
	...
	@Test
	public void users_AddFavoriteDiagnosesGroup_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = 
						"select * from  diagnosis_favorite_mstr  WHERE user_id = '"+parameters.get("userId")+"' order by create_timestamp desc ";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Users_AddFavoriteDiagnosesGroup.users_AddFavoriteDiagnosesGroup_Test(parameters, strResponse, mapPostDetails, softAssert, strCountQuery);
				}
