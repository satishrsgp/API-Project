...
import com.endpoints.Chart_GetPatientCareplanGoals;
...
public class Chart_GetPatientCareplanGoals_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientCareplanGoals_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientCareplanGoals.chart_GetPatientCareplanGoals_Test(parameters, strResponse, "goalId", softAssert);
				}
				else
				{
