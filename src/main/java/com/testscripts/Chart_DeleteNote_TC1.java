...
import com.endpoints.Chart_DeleteNote;
...
public class Chart_DeleteNote_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_DeleteNote_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						String strSQLQuery = "select case when pnx.delete_ind='Y' then 'true' else 'fasle' end as isDeleted from patient_problems pp inner join problem_note_xref pnx on pnx.problem_id=pp.uniq_id where pp.person_id='"+parameters.get("personId") + "' and pp.uniq_id='"+ parameters.get("problemId") +"' and pnx.note_id='"+parameters.get("noteId")+"'";
						...
						{
							boolTestDataStatus = Chart_DeleteNote.Chart_DeleteNote_Test(parameters, softAssert);
						}
						else
						{
