...
import com.endpoints.Chart_GetVisHistory;
...
public class Chart_GetVisHistory_TC1  extends BaseTest{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetVisHistory_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetVisHistory.chart_GetVisHistory_TC1(parameters, strResponse, "vaccineVisId", softAssert);
				}
				else
				{
