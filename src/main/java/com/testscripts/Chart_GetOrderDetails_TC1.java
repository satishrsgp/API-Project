...
import com.endpoints.Chart_GetOrderDetails;
...
public class Chart_GetOrderDetails_TC1 extends BaseTest {
	...
	@Test
	public void chart_GetOrderDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderDetails.Chart_GetOrderDetails_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
