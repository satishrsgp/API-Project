...
import com.endpoints.Provider_AddPaqSignoffComment;
...
public class Provider_AddPaqSignoffComment_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void provider_AddPaqSignoffComment_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select item_notes from signoff_queue where item_key='"+parameters.get("itemId")+"'";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Provider_AddPaqSignoffComment.provider_AddPaqSignoffComment(parameters, strResponse,mapPostDetails, softAssert, strCountQuery);
