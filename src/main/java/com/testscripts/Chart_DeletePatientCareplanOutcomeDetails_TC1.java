...
import com.endpoints.Chart_DeletePatientCareplanOutcomeDetails;
...
public class Chart_DeletePatientCareplanOutcomeDetails_TC1 extends BaseTest
{
	...
	@Test
	public void chart_DeletePatientCareplanOutcomeDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DeletePatientCareplanOutcomeDetails.Chart_DeletePatientCareplanOutcomeDetails_Test(parameters, softAssert);
					}
					else
					{
