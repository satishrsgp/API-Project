...
import com.endpoints.Resources_GetResourceAppStatHis;
...
public class Resources_GetResourceAppStatHis_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void resources_GetResourceAppStatHis_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Resources_GetResourceAppStatHis.resources_GetResourceAppStatHis_Test(parameters, strResponse, "resourceId", softAssert);
				}
				else
