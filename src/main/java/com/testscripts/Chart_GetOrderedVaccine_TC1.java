...
import com.endpoints.Chart_GetOrderedVaccine;
...
public class Chart_GetOrderedVaccine_TC1 extends BaseTest
{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderedVaccine.chart_GetOrderedVaccine_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
