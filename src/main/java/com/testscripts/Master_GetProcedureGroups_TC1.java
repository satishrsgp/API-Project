...
import com.endpoints.Master_GetProcedureGroups;
...
public class Master_GetProcedureGroups_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_GetProcedureGroups_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetProcedureGroups.master_GetProcedureGroups_Test(parameters, strResponse, "name", softAssert);
				}
				else
				{
