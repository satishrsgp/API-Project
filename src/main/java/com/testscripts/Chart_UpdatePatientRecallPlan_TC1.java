...
import com.endpoints.Chart_UpdatePatientRecallPlan;
...
public class Chart_UpdatePatientRecallPlan_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_UpdatePatientRecallPlan_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdatePatientRecallPlan.chart_UpdatePatientRecallPlan_Test(parameters, softAssert);
					}
					else
					{
