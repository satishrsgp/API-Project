...
import com.endpoints.Master_GetDiagnosisStatuses;
...
public class Master_GetDiagnosisStatuses_TC1 extends BaseTest 
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_GetDiagnosisStatuses_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetDiagnosisStatuses.master_GetDiagnosisStatuses_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
