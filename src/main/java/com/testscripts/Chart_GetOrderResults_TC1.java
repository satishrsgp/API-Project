...
import com.endpoints.Chart_GetOrderResults;
...
public class Chart_GetOrderResults_TC1 extends BaseTest
{
	...
	@Test
	public void chart_GetOrderResults_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderResults.Chart_GetOrderResults_Test(parameters, strResponse, "obxSequenceNumber", softAssert);
				}
				else
				{
