...
import com.endpoints.Chart_GetHealthConcernVitals;
...
public class Chart_GetHealthConcernVitals_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void chart_GetHealthConcernVitals_TC1()
	{
		...
		try
		{
			...
			{	
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetHealthConcernVitals.Chart_GetHealthConcernVitals_Test(parameters, strResponse, "bloodPressureHealthConcern", softAssert);
				}
				else
