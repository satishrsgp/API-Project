...
import com.endpoints.Resources_GetResourceAppointmen;
...
public class Resources_GetResourceAppointmen_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void resources_GetResourceAppointmen_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Resources_GetResourceAppointmen.resources_GetResourceAppointmen_Test(parameters, strResponse, "id", softAssert);
				}
				else
