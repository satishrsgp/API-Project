...
import com.endpoints.Chart_GetPatientAllergyReactions;
...
public class Chart_GetPatientAllergyReactions_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientAllergyReactions_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientAllergyReactions.chart_GetPatientAllergyReactions_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
