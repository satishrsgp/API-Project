...
import com.endpoints.Provider_GetProviderLabTests;
...
public class Provider_GetProviderLabTests_TC1 extends BaseTest
{
	...
	@Test
	public void provider_GetProviderLabTests_TC1()
	{
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestcaseStatus = Provider_GetProviderLabTests.Provider_GetProviderLabTests_Test(parameters, strResponse, "id", softAssert);
					}
				}
