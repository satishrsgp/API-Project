...
import com.endpoints.Chart_UpdateSeriesCompletions;
...
public class Chart_UpdateSeriesCompletions_TC1 extends BaseTest
{
	...
	@Test
	public void chart_UpdateSeriesCompletions_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdateSeriesCompletions.chart_UpdateSeriesCompletions_Test(parameters, softAssert);
					}
					else
					{
