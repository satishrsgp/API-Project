...
import com.endpoints.Chart_GetPatientMedicationById;
...
public class Chart_GetPatientMedicationById_TC1 extends BaseTest
{
	...
	@Test
	public void chart_GetPatientMedicationById_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount(BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientMedicationById.chart_GetPatientMedicationById_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
