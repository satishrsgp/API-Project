...
import com.endpoints.Master_GetAllergyReactions;
...
public class Master_GetAllergyReactions_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void master_GetAllergyReactions_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetAllergyReactions.master_GetAllergyReactions_Test(parameters, strResponse, "id", softAssert);
				}
				else
