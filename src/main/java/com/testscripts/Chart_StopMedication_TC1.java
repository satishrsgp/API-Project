...
import com.endpoints.Chart_StopMedication;
...
public class Chart_StopMedication_TC1 extends BaseTest {
	...
	@Test
	public void chart_StopMedication_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_StopMedication.chart_Stop_Test(parameters, softAssert);
					}
					else
					{
