...
import com.endpoints.Users_GetFavoriteProceduresWithOutLocation;
...
public class Users_GetFavoriteProcedures_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void users_GetFavoriteProcedures_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Users_GetFavoriteProceduresWithOutLocation.Users_GetFavoriteProceduresWithOutLocation_Test(parameters, strResponse, "code", softAssert);
				}
				else
