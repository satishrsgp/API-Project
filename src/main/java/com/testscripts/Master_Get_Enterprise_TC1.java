...
import com.endpoints.Master_Get_Enterprise;
...
public class Master_Get_Enterprise_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_Get_Enterprise_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_Get_Enterprise.master_Get_Enterprise(parameters, strResponse, "id", softAssert);
				}
				else
				{
