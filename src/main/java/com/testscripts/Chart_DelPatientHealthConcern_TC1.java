...
import com.endpoints.Chart_DelPatientHealthConcern;
...
public class Chart_DelPatientHealthConcern_TC1 extends BaseTest
{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DelPatientHealthConcern.chart_DelPatientHealthConcern(parameters, softAssert);
					}
					else
					{
