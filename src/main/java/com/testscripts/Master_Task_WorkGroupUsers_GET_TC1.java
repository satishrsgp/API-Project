...
import com.endpoints.Master_Task_WorkGroupUsers_GET;
...
public class Master_Task_WorkGroupUsers_GET_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_Task_WorkGroupUsers_GET_TC1()
	{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestDataStatus = Master_Task_WorkGroupUsers_GET.master_Task_WorkGroupUsers_GET_Test(parameters, strResponse, "userId", softAssert);
					}
					else
					{
