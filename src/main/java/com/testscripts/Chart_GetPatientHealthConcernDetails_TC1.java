...
import com.endpoints.Chart_GetPatientHealthConcernDetails;
...
public class Chart_GetPatientHealthConcernDetails_TC1 extends BaseTest
{
	//sigDescription -  json token is a calculated field need to be verified manually
		SoftAssert softAssert = new SoftAssert();

		@Test
		public void chart_GetPatientHealthConcernDetails_TC1()
		{
			...
			try
			{
				...
				for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
				{
					...
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestDataStatus = Chart_GetPatientHealthConcernDetails.chart_GetPatientHealthConcernDetails_Test(parameters, strResponse, "id", softAssert);
					}
					else
					{
