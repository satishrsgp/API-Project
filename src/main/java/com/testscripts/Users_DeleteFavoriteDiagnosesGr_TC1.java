...
import com.endpoints.Users_DeleteFavoriteDiagnosesGr;
...
public class Users_DeleteFavoriteDiagnosesGr_TC1 extends BaseTest
{
	...
	@Test
	public void users_DeleteFavoriteDiagnosesGr_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Users_DeleteFavoriteDiagnosesGr.users_DeleteFavoriteDiagnosesGr_Test(parameters, softAssert);
					}
					else
					{
