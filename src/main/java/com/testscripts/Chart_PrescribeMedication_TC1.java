...
import com.endpoints.Chart_PrescribeMedication;
...
public class Chart_PrescribeMedication_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_PrescribeMedication_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "Select uniq_id from patient_medication where person_id='"+parameters.get("personId")+"' and enc_id = '"+parameters.get("encounterId")+"'";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_PrescribeMedication.Chart_PrescribeMedication_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
