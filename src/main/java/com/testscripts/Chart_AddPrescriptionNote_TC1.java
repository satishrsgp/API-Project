...
import com.endpoints.Chart_AddPrescriptionNote;
...
public class Chart_AddPrescriptionNote_TC1 extends BaseTest{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_AddPrescriptionNote_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select note_id from med_rx_notes where medication_id='" + parameters.get("medicationId") + "'";
				...
				{
					boolTestDataStatus = Chart_AddPrescriptionNote.chart_AddPrescriptionNote(parameters, mapPostDetails, softAssert, strCountQuery);
				}
