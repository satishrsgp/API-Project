...
import com.endpoints.Persons_GetFormularyCopay;
...
public class Persons_GetFormularyCopay_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void persons_GetFormularyCopay_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				mapRequestParrameters.put("strQueryString", "?pbmId="+parameters.get("pbmId1"));
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Persons_GetFormularyCopay.Persons_GetFormularyCopay_Test(parameters, strResponse, "sourceId", softAssert);
				}
				else
				{
