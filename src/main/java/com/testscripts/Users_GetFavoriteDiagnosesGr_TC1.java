...
import com.endpoints.Users_GetFavoriteDiagnosesGr;
...
public class Users_GetFavoriteDiagnosesGr_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void users_GetFavoriteDiagnosesGr_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Users_GetFavoriteDiagnosesGr.users_GetFavoriteDiagnosesGr_Test(parameters, strResponse, "id", softAssert);
				}
				else
