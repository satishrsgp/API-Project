...
import com.endpoints.Audit_GetPatientEducationDetail;
...
public class Audit_GetPatientEducationDetail_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void audit_GetPatientEducationDetail_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Audit_GetPatientEducationDetail.audit_GetPatientEducationDetail(parameters, strResponse, "id", softAssert);
				}
				else
				{
