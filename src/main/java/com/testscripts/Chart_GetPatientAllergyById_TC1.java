...
import com.endpoints.Chart_GetPatientAllergyById;
...
public class Chart_GetPatientAllergyById_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientAllergyById_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientAllergyById.chart_GetPatientAllergyById_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
