...
import com.endpoints.Chart_UpdateInterventionDetails;
...
public class Chart_UpdateInterventionDetails_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_UpdateInterventionDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdateInterventionDetails.Chart_UpdateInterventionDetails_Test(parameters, softAssert);
					}
					else
					{
