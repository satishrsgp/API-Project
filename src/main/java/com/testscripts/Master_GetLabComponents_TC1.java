...
import com.endpoints.Master_GetLabComponents;
...
public class Master_GetLabComponents_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void master_GetLabComponents_TC1()
	{
		...
		try
		{
			...
			{	
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetLabComponents.master_GetLabComponents_Test(parameters, strResponse, "testId", softAssert);
				}
				else
