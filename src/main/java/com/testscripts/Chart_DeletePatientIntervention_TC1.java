...
import com.endpoints.Chart_DeletePatientIntervention;
...
public class Chart_DeletePatientIntervention_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_DelPatientHealthConcern_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DeletePatientIntervention.chart_DeletePatientIntervention(parameters, softAssert);
					}
					else
					{
