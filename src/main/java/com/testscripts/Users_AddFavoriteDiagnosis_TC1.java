...
import com.endpoints.Users_AddFavoriteDiagnosis;
...
public class Users_AddFavoriteDiagnosis_TC1 extends BaseTest
{
	...
	@Test
	public void users_AddFavoriteDiagnosis_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = 
						"select diagnosis_code_id,diag_description from  diagnosis_favorite_xref  WHERE diag_favorite_id = '"+parameters.get("groupId")+"' order by create_timestamp desc ";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus =Users_AddFavoriteDiagnosis.users_AddFavoriteDiagnosis_Test(parameters, strResponse, mapPostDetails, softAssert, strCountQuery);
				}
