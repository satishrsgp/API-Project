...
import com.endpoints.Chart_GetVaccineComponentLotNumbers;
...
public class Chart_GetVaccineComponentLotNumbers_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetVaccineComponentLotNumbers_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetVaccineComponentLotNumbers.chart_GetVaccineComponentLotNumbers_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
