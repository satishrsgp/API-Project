...
import com.endpoints.Chart_GetOrderedVaccineDiagnosis;
...
public class Chart_GetOrderedVaccineDiagnosis_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void chart_GetOrderedVaccineDiagnosis_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderedVaccineDiagnosis.Chart_GetOrderedVaccineDiagnosis_Test(parameters, strResponse, "id", softAssert);
				}
				else
	...
	//Don't know the purpose but unsed code so commenting it (Anil Kumar Veluru)
	/*private boolean Chart_GetOrderedVaccineDiagnosis_Test(
			HashMap<String, String> parameters, String strResponse,
			String string, SoftAssert softAssert2) {
		return false;
	}*/
