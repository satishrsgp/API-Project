...
import com.endpoints.Chart_SendErx;
...
public class Chart_SendErx_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_SendErx_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select response_type  as requestType from erx_message_history " 
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 200)
				{
					...
					boolTestDataStatus = Chart_SendErx.chart_SendErx(parameters, strResponse,mapPostDetails, softAssert, strCountQuery);
