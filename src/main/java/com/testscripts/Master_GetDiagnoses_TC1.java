...
import com.endpoints.Master_GetDiagnoses;
...
public class Master_GetDiagnoses_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_GetDiagnoses_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetDiagnoses.master_GetDiagnoses_Test(parameters, strResponse, "diagnosisCodeId", softAssert);
				}
				else
				{
