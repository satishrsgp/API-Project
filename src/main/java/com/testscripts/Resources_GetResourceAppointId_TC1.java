...
import com.endpoints.Resources_GetResourceAppointId;
...
public class Resources_GetResourceAppointId_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void resources_GetResourceAppointId_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Resources_GetResourceAppointId.resources_GetResourceAppointId_Test(parameters, strResponse, "id", softAssert);
				}
				else
