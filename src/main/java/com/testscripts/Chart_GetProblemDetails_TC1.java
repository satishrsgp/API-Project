...
import com.endpoints.Chart_GetProblemDetails;
...
public class Chart_GetProblemDetails_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetProblemDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetProblemDetails.Chart_GetProblemDetails_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
