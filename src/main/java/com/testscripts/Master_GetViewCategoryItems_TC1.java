...
import com.endpoints.Master_GetViewCategoryItems;
...
public class Master_GetViewCategoryItems_TC1 extends BaseTest
{
	...
	//sigDescription -  json token is a calculated field need to be verified manually
		...
		@Test
		public void master_GetViewCategoryItems_TC1()
		{
			...
			try
			{
				...
				for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
				{
					...
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestDataStatus = Master_GetViewCategoryItems.master_GetViewCategoryItems_Test(parameters, strResponse, "categoryId", softAssert);
					}
					else
					{
