...
import com.endpoints.Persons_AuditApiAccess;
...
public class Persons_AuditApiAccess_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void persons_AuditApiAccess_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select * from ext_api_access_data  WHERE person_id = '"+parameters.get("personId")+"' order by create_timestamp desc";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 200)
				{
					...
					boolTestDataStatus = Persons_AuditApiAccess.persons_AuditApiAccess_Test(parameters, strResponse, mapPostDetails, softAssert, strCountQuery);
				}
				else if(objHandler.getIntResponseCode() == 201)
