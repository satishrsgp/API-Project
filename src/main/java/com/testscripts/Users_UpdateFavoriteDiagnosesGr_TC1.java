...
import com.endpoints.Users_UpdateFavoriteDiagnosesGr;
...
public class Users_UpdateFavoriteDiagnosesGr_TC1 extends BaseTest
{
	...
	@Test
	public void users_UpdateFavoriteDiagnosesGr_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Users_UpdateFavoriteDiagnosesGr.Users_UpdateFavoriteDiagnosesGr_Test(parameters, softAssert);
					}
					else
					{
