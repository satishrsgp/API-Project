...
import com.endpoints.Master_SearchPatientProblems;
...
public class Master_SearchPatientProblems_TC1 extends BaseTest {
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_SearchPatientProblems_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_SearchPatientProblems.master_SearchPatientProblems_Test(parameters, strResponse, "fullySpecifiedName", softAssert);
				}
				else
				{
