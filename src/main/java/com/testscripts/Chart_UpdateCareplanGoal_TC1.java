...
import com.endpoints.Chart_UpdateCareplanGoal;
...
public class Chart_UpdateCareplanGoal_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_UpdateCareplanGoal_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdateCareplanGoal.Chart_UpdateCareplanGoal_Test(parameters, softAssert);
					}
					else
					{
