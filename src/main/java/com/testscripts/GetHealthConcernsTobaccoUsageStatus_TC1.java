...
import com.endpoints.GetHealthConcernsTobaccoUsageStatus;
...
public class GetHealthConcernsTobaccoUsageStatus_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void getHealthConcernsTobaccoUsageStatus_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = GetHealthConcernsTobaccoUsageStatus.getHealthConcernsTobaccoUsageStatus_Test(parameters, strResponse, "personId", softAssert);
				}
				else
				{
