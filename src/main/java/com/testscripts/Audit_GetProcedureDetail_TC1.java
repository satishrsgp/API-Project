...
import com.endpoints.Audit_GetProcedureDetail;
...
public class Audit_GetProcedureDetail_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void audit_GetProcedureDetail_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Audit_GetProcedureDetail.audit_GetProcedureDetail(parameters, strResponse, "id", softAssert);
				}
				else
				{
