...
import com.endpoints.Master_GetPharmacies;
...
public class Master_GetPharmacies_TC1 extends BaseTest
{
	...
	@Test
	public void master_GetPharmacies_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetPharmacies.master_GetPharmacies_Test(parameters, strResponse, "zip", softAssert);
				}
				else
				{
