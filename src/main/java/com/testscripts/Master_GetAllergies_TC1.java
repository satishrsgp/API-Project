...
import com.endpoints.Master_GetAllergies;
...
public class Master_GetAllergies_TC1 extends BaseTest
{
	...
	@Test
	public void master_GetAllergies_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetAllergies.master_GetAllergies_Test(parameters, strResponse, "allergyId", softAssert);
				}
				else
				{
