...
import com.endpoints.Chart_AddOrderedVaccine;
...
public class Chart_AddOrderedVaccine_TC1 extends BaseTest
{
	...
	@Test
	public void chart_AddOrderedVaccine_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "Select * from imm_order_vaccines where order_num='"+parameters.get("orderId")+"'";
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_AddOrderedVaccine.Chart_AddOrderedVaccine_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
