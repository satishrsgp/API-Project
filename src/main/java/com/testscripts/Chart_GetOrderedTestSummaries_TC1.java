...
import com.endpoints.Chart_GetOrderedTestSummaries;
import com.endpoints.Chart_GetPatientEncounterDiagnoses;
...
public class Chart_GetOrderedTestSummaries_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void chart_GetOrderedTestSummaries_TC1()
	{
		...
		try
		{
			...
			{	
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderedTestSummaries.chart_GetOrderedTestSummaries(parameters, strResponse, "id", softAssert);
				}
				else
