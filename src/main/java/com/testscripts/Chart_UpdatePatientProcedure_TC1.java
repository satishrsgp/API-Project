...
import com.endpoints.Chart_UpdatePatientProcedure;
...
public class Chart_UpdatePatientProcedure_TC1 extends BaseTest
{
	...
	@Test
	public void chart_UpdatePatientProcedure_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdatePatientProcedure.chart_UpdatePatientProcedure_Test(parameters, softAssert);
					}
					else
					{
