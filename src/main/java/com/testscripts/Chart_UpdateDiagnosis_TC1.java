...
import com.endpoints.Chart_UpdateDiagnosis;
...
public class Chart_UpdateDiagnosis_TC1 extends BaseTest 
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_UpdateDiagnosis_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(objHandler.getIntResponseCode() == 200)
				{
						...
						boolTestDataStatus = Chart_UpdateDiagnosis.chart_UpdateDiagnosis_Test(parameters, softAssert);
				}
				}
				else
