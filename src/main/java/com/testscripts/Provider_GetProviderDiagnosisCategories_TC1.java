...
import com.endpoints.Provider_GetProviderDiagnosisCategories;
...
public class Provider_GetProviderDiagnosisCategories_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void provider_GetProviderDiagnosisCategories_TC1()
	{
			...
			{	
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestcaseStatus = Provider_GetProviderDiagnosisCategories.provider_GetProviderDiagnosisCategories(parameters, strResponse, "providerId", softAssert);
					}
				}
