...
import com.endpoints.Prvdr_GtPrvdrDiagnsesByCatgry;
...
public class Prvdr_GtPrvdrDiagnsesByCatgry_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void prvdr_GtPrvdrDiagnsesByCatgry_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Prvdr_GtPrvdrDiagnsesByCatgry.prvdr_GtPrvdrDiagnsesByCatgry_Test(parameters, strResponse, "rank", softAssert);
				}
				else
				{
