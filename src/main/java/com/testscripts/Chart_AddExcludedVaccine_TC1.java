...
import com.endpoints.Chart_AddExcludedVaccine;
...
public class Chart_AddExcludedVaccine_TC1 extends BaseTest {
	...
	@Test
	public void chart_AddExcludedVaccine_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_AddExcludedVaccine.Chart_AddExcludedVaccine_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
