...
import com.endpoints.Chart_DeleteObservationPanel;
...
public class Chart_DeleteObservationPanel_TC1 extends BaseTest
{
	...
	@Test
	public void chart_DeleteObservationPanel_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DeleteObservationPanel.chart_DeleteObservationPanel(parameters, softAssert);
					}
					else
					{
