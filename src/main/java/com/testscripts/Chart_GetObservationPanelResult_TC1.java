...
import com.endpoints.Chart_GetObservationPanelResult;
...
public class Chart_GetObservationPanelResult_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetObservationPanelResult_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetObservationPanelResult.Chart_GetObservationPanelResult_Test(parameters,strResponse, "observationSequenceNumber", softAssert);
				}
				else
				{
