...
import com.endpoints.Favorites_GetCustomDosageOrders;
...
public class Favorites_GetCustomDosageOrders_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void favorites_GetCustomDosageOrders_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Favorites_GetCustomDosageOrders.favorites_GetCustomDosageOrders_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
