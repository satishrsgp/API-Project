...
import com.endpoints.Chart_UpdatePatCPOutcmDetails;
...
public class Chart_UpdatePatCPOutcmDetails_TC1 extends BaseTest
{
	...
	@Test
	public void updatePatCPOutcmDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdatePatCPOutcmDetails.updatePatCPOutcmDetails(parameters, softAssert);
					}
					else
					{
