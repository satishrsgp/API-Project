...
import com.endpoints.Tasking_GetFavoriteAssignees;
...
public class Tasking_GetFavoriteAssignees_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void tasking_GetFavoriteAssignees_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Tasking_GetFavoriteAssignees.tasking_GetFavoriteAssignees_Test(parameters, strResponse, "id", softAssert);
				}
				else
