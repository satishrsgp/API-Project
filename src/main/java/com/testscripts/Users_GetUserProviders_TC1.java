...
import com.endpoints.Users_GetUserProviders;
 ...
 * ******************************************************************
 */
public class Users_GetUserProviders_TC1 extends BaseTest {


	SoftAssert softAssert = new SoftAssert();

	@Test
	public void users_GetUserProviders_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Users_GetUserProviders.users_GetUserProviders_Test(parameters, strResponse, "userId", softAssert);
				}
				else
				{
