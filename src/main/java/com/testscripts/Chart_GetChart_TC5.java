...
import com.endpoints.Person_Documents_GET;
...
public class Chart_GetChart_TC5 extends BaseTest
{
	/*Odata not validated so not tested in V2.0 execution */
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetChart_TC5()
	{
		...
		try
		{
			...
			{	
				...
				mapRequestParrameters.put("strQueryString", "?$expand=Documents");
				...
				if(!objHandler.getStrResponse().isEmpty())
				{
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						{
							String strResponseDocumentPart = JsonPath.read(strResponse, "$..documents").toString();
