...
import com.endpoints.Chart_GetOrderSchedule;
...
public class Chart_GetOrderSchedule_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetOrderSchedule_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderSchedule.Chart_GetOrderSchedule_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
