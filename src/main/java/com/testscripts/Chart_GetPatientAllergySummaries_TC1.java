...
import com.endpoints.Chart_GetPatientAllergySummaries;
...
public class Chart_GetPatientAllergySummaries_TC1 extends BaseTest
{
	...
	//sigDescription -  json token is a calculated field need to be verified manually
		...
		@Test
		public void chart_GetPatientAllergySummaries_TC1()
		{
			...
			try
			{
				...
				for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
				{
					...
					if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
					{
						...
						boolTestDataStatus = Chart_GetPatientAllergySummaries.chart_GetPatientAllergySummaries_Test(parameters, strResponse, "id", softAssert);
					}
					else
					{
