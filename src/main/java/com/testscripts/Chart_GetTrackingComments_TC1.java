...
import com.endpoints.Chart_GetTrackingComments;
...
public class Chart_GetTrackingComments_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetTrackingComments_TC1()
	{
		...
		try
		{
			...
			{	
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetTrackingComments.chart_GetTrackingComments_Test(parameters, strResponse, "id", softAssert);
				}
				else
