...
import com.endpoints.Chart_CreateCPHealthConcern;
...
public class Chart_CreateCPHealthConcern_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_CreateCPHealthConcern_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select * from health_concern_ext_ "
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_CreateCPHealthConcern.chart_CreateCPHealthConcern(parameters, strResponse,mapPostDetails, softAssert, strCountQuery);
