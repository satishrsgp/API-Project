...
import com.endpoints.Chart_AddInterventionDetails;
...
public class Chart_AddInterventionDetails_TC1 extends BaseTest
{
	...
	@Test
	public void chart_AddInterventionDetails_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass") && objHandler.getIntResponseCode() == 201)
				{
					...
					boolTestDataStatus = Chart_AddInterventionDetails.Chart_AddInterventionDetails_Test(parameters,strResponse, mapPostDetails, softAssert, strCountQuery);
				}
