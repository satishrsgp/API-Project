...
import com.endpoints.GetPatientAllergySummaries;
...
public class GetPatientAllergySummaries_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void getPatientAllergySummaries_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = GetPatientAllergySummaries.getPatientAllergySummaries_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
