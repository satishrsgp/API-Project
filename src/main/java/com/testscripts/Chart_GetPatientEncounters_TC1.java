...
import com.endpoints.Chart_GetPatientEncounters;
...
public class Chart_GetPatientEncounters_TC1 extends BaseTest
{
	...
	@Test
	public void chart_GetPatientEncounters_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientEncounters.chart_GetPatientEncounters_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
