...
import com.endpoints.Chart_GetMedicationDosageRange;
...
public class Chart_GetMedicationDosageRange_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetMedicationDosageRange_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetMedicationDosageRange.chart_GetMedicationDosageRange_Test(parameters, strResponse, "minimumAge", softAssert);
				}
				else
				{
