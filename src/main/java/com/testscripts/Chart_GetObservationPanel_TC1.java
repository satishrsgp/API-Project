...
import com.endpoints.Chart_GetObservationPanel;
...
public class Chart_GetObservationPanel_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetObservationPanel_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetObservationPanel.Chart_GetObservationPanel_Test(parameters,strResponse, "id", softAssert);
				}
				else
				{
