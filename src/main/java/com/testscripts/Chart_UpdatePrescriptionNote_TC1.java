...
import com.endpoints.Chart_UpdatePrescriptionNote;
...
public class Chart_UpdatePrescriptionNote_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_UpdatePatientProblem_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdatePrescriptionNote.chart_UpdatePrescriptionNote(parameters, softAssert);
					}
					else
					{
