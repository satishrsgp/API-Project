...
import com.endpoints.Chart_GetOrderedVaccineDiagnosi;
...
public class Chart_GetOrderedVaccineDiagnosi_TC1 extends BaseTest
{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetOrderedVaccineDiagnosi.Chart_GetOrderedVaccineDiagnosi_Test(parameters, strResponse, "id", softAssert);
				}
				else
