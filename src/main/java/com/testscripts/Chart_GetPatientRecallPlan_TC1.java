...
import com.endpoints.Chart_GetPatientRecallPlan;
...
public class Chart_GetPatientRecallPlan_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetPatientRecallPlan_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetPatientRecallPlan.chart_GetPatientRecallPlan_Test(parameters, strResponse, "personId", softAssert);
				}
				else
				{
