...
import com.endpoints.Users_GetFavoriteLocProcedures;
...
public class Users_GetFavoriteLocProcedures_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void users_GetFavoriteLocProcedures_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus =Users_GetFavoriteLocProcedures.users_GetFavoriteLocProcedures_Test(parameters, strResponse, "code", softAssert);
				}
				else
