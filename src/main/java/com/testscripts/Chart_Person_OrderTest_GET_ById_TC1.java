...
import com.endpoints.Chart_Person_OrderTest_GET_ById;
...
public class Chart_Person_OrderTest_GET_ById_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_Person_OrderTest_GET_ById_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_Person_OrderTest_GET_ById.chart_Person_OrderTest_GET_ById_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
