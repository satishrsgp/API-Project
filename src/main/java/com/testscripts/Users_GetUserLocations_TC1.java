...
import com.endpoints.Users_GetUserLocations;
...
public class Users_GetUserLocations_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void users_GetUserLocations_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Users_GetUserLocations.users_GetUserLocations_Test(parameters, strResponse, "userId", softAssert);
				}
				else
				{
