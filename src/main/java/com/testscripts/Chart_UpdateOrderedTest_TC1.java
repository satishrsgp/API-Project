...
import com.endpoints.Chart_UpdateOrderedTest;
...
public class Chart_UpdateOrderedTest_TC1 extends BaseTest
{
	...
	@Test
	public void chart_UpdateOrderedTest_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_UpdateOrderedTest.chart_UpdateOrderedTest_Test(parameters, softAssert);
					}
					else
					{
