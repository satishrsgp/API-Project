...
import com.endpoints.Persons_GetEligibilities;
...
public class Persons_GetEligibilities_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();
	
	@Test
	public void persons_GetEligibilities_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Persons_GetEligibilities.persons_GetEligibilities_Test(parameters, strResponse, "personId", softAssert);
				}
				else
