...
import com.endpoints.Chart_DeletePatientProcedure;
...
public class Chart_DeletePatientProcedure_TC1 extends BaseTest
{
	...
	@Test
	public void chart_DeletePatientProcedure_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equals(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					{
						boolTestDataStatus = Chart_DeletePatientProcedure.Chart_DeletePatientProcedure_Test(parameters, softAssert);
					}
					else
					{
