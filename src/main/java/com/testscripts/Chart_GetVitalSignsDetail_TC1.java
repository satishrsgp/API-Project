...
import com.endpoints.Chart_GetVitalSignsDetail;
...
public class Chart_GetVitalSignsDetail_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_GetVitalSignsDetail_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Chart_GetVitalSignsDetail.chart_GetVitalSignsDetail_Test(parameters, strResponse, "id", softAssert);
				}
				else
				{
