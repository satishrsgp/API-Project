...
import com.endpoints.Master_GetVaccineManufacturers;
...
public class Master_GetVaccineManufacturers_TC1 extends BaseTest
{

	SoftAssert softAssert = new SoftAssert();

	@Test
	public void master_GetVaccineManufacturers_TC1()
	{
		...
		try
		{
			...
			if(!objHandler.getStrResponse().isEmpty())
			{
				if(Objects.equal(testexecutionResult[testexecutionResult.length-1].toString().toLowerCase(), "pass"))
				{
					...
					boolTestDataStatus = Master_GetVaccineManufacturers.master_GetVaccineManufacturers_Test(parameters, strResponse, "manufacturerNumber", softAssert);
				}
				else
				{
