...
import com.endpoints.Chart_AddPatientProblem;
...
public class Chart_AddPatientProblem_TC1 extends BaseTest
{
	SoftAssert softAssert = new SoftAssert();

	@Test
	public void chart_AddPatientProblem_TC1()
	{
		...
		try
		{
			...
			for(int k=1; k<=ExcelReaderWriter.getRowCount( BaseTest.mapExecutionConfigs.get("TestDataWorkBook"), strTestDataSheetName); k++)
			{
				...
				String strCountQuery = "select uniq_id from patient_problems where person_id='" + parameters.get("personId") + "' and delete_ind='N'";
				...
				{
					boolTestDataStatus = Chart_AddPatientProblem.Chart_AddPatientProblem_Test(parameters, mapPostDetails, softAssert, strCountQuery);
				}
